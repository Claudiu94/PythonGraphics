...
def plotNewlyBuildApparmentsProjection(startIndex, endIndex):
	for codeIndex in range(startIndex, endIndex):
		try:
			dataFrame = dataFrameService.getNewlyBuildAppartmentData(regionCodes[codeIndex])
			dataFrame1 = dataFrameService.getNumberOfSoldHouses(regionCodes[codeIndex])
			y1 = list(dataFrame["SMÅHUS"])
			y2 = list(dataFrame["FLERBO"])
			lenP = len(dataFrame1["keys"])
			proj_y1 = list(dataFrame1["SMÅHUS"])[lenP - 8 : lenP]
			proj_y2 = list(dataFrame1["FLERBO"])[lenP - 8 : lenP]
			projYears = ["2018K4", "2019K1", "2019K2", "2019K3", "2019K4", "2020K1", "2020K2", "2020K3", "2020K4"]
			...
			y3 = [y1[len(y1) - 1]]
			y4 = [y2[len(y2) - 1]]
			for i in range(1, len(proj_y1) + 1):
				y3.append(y3[i-1] + proj_y1[i - 1])
				y4.append(y4[i-1] + proj_y2[i - 1])

			trace0 = go.Scatter(y = y2, x = dataFrame["keys"], name = "Flerbostadshus", line = dict(color = 'blue'), marker = dict(color = "blue", opacity = 0.2, line = dict(color = "blue")), fill = 'tonexty')
			trace1 = go.Scatter(y = y1, x = dataFrame["keys"], name = "Småhus", line = dict(color = 'red'), marker = dict(color = "red", opacity = 0.2, line = dict(color = "red")), fill = 'tonexty')
			trace2 = go.Scatter(y = y4, x = projYears, name = "Projected flerbostadshus", line = dict(color = 'yellow'))
